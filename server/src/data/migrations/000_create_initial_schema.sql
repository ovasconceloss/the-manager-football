...
CREATE TABLE competition (
    ...
    has_group_stage INTEGER NOT NULL DEFAULT 0 has_knockout_stage INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE attribute_type (
    ...
    category TEXT NOT NULL CHECK(
        category IN ('technical', 'physical', 'mental', 'goalkeeper')
    )
);
...
CREATE TABLE financial_transaction (
    ...
    -- FK for player (e.g. player sale/purchase, player 
    related_staff_id INTEGER NULL,
    -- FK for staff (e.g. staff salary)
    related_match_id INTEGER NULL,
    -- FK for match (e.g. match ticket revenue)
    related_club_id INTEGER NULL,
    -- FK for club (e.g. transfers between clubs)
    FOREIGN KEY (club_id) REFERENCES club(id),
...
CREATE TABLE club_budget (
    ...
    UNIQUE (club_id, season_id) FOREIGN KEY (club_id) REFERENCES club(id),
    FOREIGN KEY (season_id) REFERENCES season(id)
);
-- club_budget indexes
...
CREATE TABLE tactic_instruction (
    ...
    value TEXT NOT NULL,
    -- The value of instruction (ex: 'High', ' 'Short', '5')
...
CREATE TABLE player_injury (
    ...
    severity TEXT NOT NULL CHECK(
        severity IN ('Minor', 'Moderate', 'Severe', 'Career-Ending')
    ),
...
INSERT INTO suspension_reason_type (name)
VALUES ('Red Card'),
    ...
    ('Match Fixing');
-- injury_type insert
INSERT INTO injury_type (name, typical_recovery_days)
VALUES ('Muscle Strain', 14),
    ('Ankle Sprain', 21),
    ('ACL Tear', 270),
    ('Hamstring Injury', 45),
    ('Concussion', 7),
...
INSERT INTO player_position (name)
VALUES ('GK'),
    -- Goalkeeper
    ('CB'),
    -- Centre-Back
...
INSERT INTO tactic_instruction_type (name, value_type)
VALUES ('Defensive Line', 'text'),
    -- Ex: 'High', 'Medium', 'Low'
    ('Pressing Intensity', 'text'),
    -- Ex: 'High', 'Medium', 'Low'
    ('Passing Style', 'text'),
    -- Ex: 'Short', 'Mixed', 'Long'
    ('Tempo', 'text'),
    -- Ex: 'Slow', 'Normal', 'Fast'
    ('Width', 'text'),
    -- Ex: 'Narrow', 'Normal', 'Wide'
    ('Mentality', 'text');
-- Ex: 'Attacking', 'Balanced', 'Defensive'
-- transfer_window_type insert
INSERT INTO transfer_window_type (name)
VALUES ('Summer'),
    ('Winter'),
    ('Emergency');
-- individual_award_type insert
INSERT INTO individual_award_type (
        name,
        description,
        category,
        is_competition_specific
    )
VALUES (
        'Ballon d''Or',
    ...
    (
        'Golden Boot',
        'Top scorer in national leagues this season.',
        'Player',
        0
    ),
    (
    ...
    (
        'Best Player of Competition',
        'Best player in a specific competition.',
        'Player',
        1
    ),
    (
        'Top Scorer of Competition',
        'Top scorer in a specific competition.',
        'Player',
        1
    ),
    (
        'Team of the Year',
        'Selection of the best players of the year.',
        'Team Selection',
        0
    );
-- formation insert
INSERT INTO formation (name, formation_string)
VALUES ('4-4-2 Flat', '4-4-2'),
    ('4-3-3 Attacking', '4-3-3'),
    ('3-5-2', '3-5-2'),
    ('4-2-3-1', '4-2-3-1'),
    ('5-3-2 Defensive', '5-3-2');
-- insert staff_attribute_type
INSERT INTO staff_attribute_type (name, category)
VALUES ('Coaching - Attacking', 'Coaching'),
...
INSERT INTO transaction_category (name, transaction_type_id)
VALUES (
        'Ticket Sales',
        (
            SELECT id
...
VALUES ('Permanent'),
    -- Permanent transfer with or without cost
    ('Loan'),
    -- Player loan
    ('Free Transfer'),
    -- Transfer without transfer fee (contract expired)
    ('Retirement'),
    -- Player retired
    ('Release');
-- The player was released from his contract by the club
-- insert staff function types
INSERT INTO staff_function_type (name)
VALUES ('Manager'),
    ('Assistant Manager'),
    ('Head Coach'),
    ('Goalkeeping Coach'),
...
INSERT INTO tactical_style_type (name, description)
VALUES (
        'Attacking',
        'Focus on an attacking, high-pressure game.'
    ),
    (
        'Defensive',
        'Prioritizes defensive solidity and counter-attacks.'
    ),
    (
        'Possession-based',
        'Seek to control possession of the ball.'
    ),
    (
        'Counter-attacking',
        'Specializes in fast transitions and counter-attacks.'
    ),
    (
        'Direct',
        'Prefers long, direct passes to the attack.'
    );
-- insert attribute types
INSERT INTO attribute_type (name, category)
VALUES ('Finishing', 'technical'),
    -- Ability to score goals
    ('Passing', 'technical'),
    -- Quality and precision of passes
    ('Dribbling', 'technical'),
    -- Ability to dribble and control the ball
    ('Heading', 'technical'),
    -- Skilled in aerial disputes and head shots
    ('Tackling', 'defensive'),
    -- Ability to steal the ball and disarm opponents
    ('Marking', 'defensive'),
    -- Ability to follow and nullify opponents
    ('Pace', 'physical'),
    -- Speed and acceleration
    ('Strength', 'physical'),
    -- Ability to win physical duels
    ('Stamina', 'physical'),
    -- Breath and ability to play 90 minutes
    ('Vision', 'mental'),
    -- Ability to read the game and find spaces
    ('Leadership', 'mental'),
    -- Ability to inspire and guide the team
    ('Composure', 'mental'),
    -- Ability to remain calm under pressure
    ('Goalkeeping', 'goalkeeper');
-- This includes reflexes, goalkeeping, positioning, etc.
-- Inserting cities for Portugal
INSERT INTO city (nation_id, name)
VALUES (
        (
            SELECT id
            FROM nation
